<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="提问日期" prop="tiwenriqi">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.tiwenriqi" 
                type="date"
                placeholder="提问日期">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.tiwenriqi" label="提问日期" prop="tiwenriqi">
              <el-input v-model="ruleForm.tiwenriqi" 
                placeholder="提问日期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="提问内容" prop="tiwenneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.tiwenneirong" label="提问内容" prop="tiwenneirong">
                    <span v-html="ruleForm.tiwenneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  created() {
	this.ruleForm.tiwenriqi = this.getCurDate()
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xueshengxingming!=''&&json.xueshengxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jiaoshiyonghu/jiaoshigonghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    jiaoshigonghaoChange () {
      this.$http({
        url: `follow/jiaoshiyonghu/jiaoshigonghao?columnValue=`+ this.ruleForm.jiaoshigonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.kemuleixing){
	...
	this.ruleForm.tiwenneirong = this.ruleForm.tiwenneirong.replace(reg,'../../../springboot0b6e5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xueshengtiwen/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xueshengtiwenCrossAddOrUpdateFlag = false;
                  this.parent.search();
